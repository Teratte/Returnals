...
public class Machinegun : WeaponBase
{
    ...
    [SerializeField]
    private float inaccuracyDitance = 3.0f;
    ...
    private void Awake()
    {
        ...
        {
            GameManager.instance.currentMachinegunAmmo = weaponSetting.maxCapacity;
            GameManager.instance.isFirstStage = false;
        }

        CurrentAmmo = GameManager.instance.currentMachinegunAmmo;
    ...
    public void OnAttack()
    {
        ...
        if (Time.time >= lastAttackTime + weaponSetting.fireRate)
        {
            ...
            // 남은 탄알이 없으면 발사 불가능
            if (GameManager.instance.currentMachinegunAmmo <= 0)
                return;
            if(!onSubMagazine)
                GameManager.instance.currentMachinegunAmmo--;
            CurrentAmmo = GameManager.instance.currentMachinegunAmmo;
            // 발사 이펙트 재생
            ShotEffect();
            // 발사 사운드 재생
    ...
    public void Reload()
    {
        if (GameManager.instance.maxMachinegunAmmo <= 0 || GameManager.instance.currentMachinegunAmmo >= weaponSetting.maxCapacity)
        {
    ...
    private IEnumerator ReloadRoutine()
    {
        ...
        // 탄창에 채울 탄알 계산
        int ammoToFill = weaponSetting.maxCapacity - GameManager.instance.currentMachinegunAmmo;

        // 탄창에 채워야 할 탄알이 남은 탄알보다 많다면
        // 채워야 할 탄알 수를 남은 탄알 수에 맞춰 줄임
        if (GameManager.instance.maxMachinegunAmmo <= ammoToFill)
        {
            ammoToFill = GameManager.instance.maxMachinegunAmmo;
        }

        // 탄창을 채움
        GameManager.instance.currentMachinegunAmmo += ammoToFill;
        CurrentAmmo = GameManager.instance.currentMachinegunAmmo;
        // 남은 탄알에서 탄창에 채운만큼 탄알을 뺌
        GameManager.instance.maxMachinegunAmmo -= ammoToFill;
        MaxAmmo = GameManager.instance.maxMachinegunAmmo;
        // 총의 상태를 발사 준비 상태로 변경
        isReload = false;
    }
